...
def ingresar_ticket(request):
    ...
    try:
        ...
        encargado_cliente = EncargadoCliente.objects.get(
            codUsuario__pk=request.user.pk)
        formulario_ingreso.fields["Proyecto"] = forms.ModelChoiceField(
            queryset=Proyecto.objects.filter(cliente__pk=encargado_cliente.cliente.pk))
    ...
    try:
        if request.POST:
            ...
            ticket.Proyecto = Proyecto.objects.get(
                pk=request.POST.get('Proyecto'))
            ...
            ticket.titulo = None if request.POST.get(
                'titulo') == '' else request.POST.get('titulo')

            ticket.descripcion_ticket = None if request.POST.get(
                'descripcion_ticket') == '' else request.POST.get('descripcion_ticket')
            ticket.comentario = None if request.POST.get(
                'comentario') == '' else request.POST.get('comentario')
            ticket.prioridad = None if request.POST.get(
                'prioridad') == '' else request.POST.get('prioridad')
...
def ticket_editar(request, id_ticket):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            ticket.UsuarioModificador = request.user
            if request.user.is_superuser:
                ticket.asignadoA = User.objects.get(
                    pk=request.POST.get('asignadoA'))
                ticket.codEstado = Estado.objects.get(
                    pk=request.POST.get('codEstado'))
